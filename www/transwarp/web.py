...
import threading
import jinja2
import re, datetime, time, urllib, os, mimetypes, cgi
import logging, functools, traceback, sys, types
...
def favicon_handler():
    '''
    仅调试用，实际部署的时候让nginx去搞定/favicon.ico
    浏览器会自动请求/favicon.ico获取网站图标
    '''
...
class Request(object):
    ...
    def __init__(self, environ):
        '''
        environ  wsgi处理函数里面的那个environ
        wsgi server 调用wsgi处理函数时传入的
        包含了用户请求的所有数据
        '''
    ...
    def _parse_input(self):
        ...
        比如：Request({'REQUEST_METHOD':'POST', 'wsgi.input':StringIO('a=1&b=M%20M&c=ABC&d=XYZ&e=')})
...
#注意, 这里的 正则 是表示: 匹配非 * ? 的字符, ^在 [^x]的意义是匹配除了x意外的字符
...
def _build_pattern_fn(pattern):
    ...
    if m:
        return lambda p: p.startswith(m.group(1))   # 用于判断 传入的 字符串 是不是以所定义的拦截路径开头
